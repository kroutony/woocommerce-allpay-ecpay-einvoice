...
// 執行發票作業項目。
abstract class E_InvoiceMethod
{
	// 一般開立發票。
...
// 通關方式
abstract class E_ClearanceMark
{
	// 經海關出口
	const Yes = '1';
	// 非經海關出口
...
// 延遲註記
abstract class E_DelayFlagType
{
	// 延遲註記
	const Delay = '1';
	// 觸發註記
...
// 交易類別
abstract class E_PayTypeCategory
{
	// ECBANK
	const Ecbank = '1';
	// ECPAY
...
// 通知類別 
abstract class E_AllowanceNotifyType
{
	// 簡訊通知
	const Sms = 'S';
	// 電子郵件通知
...
// 發送內容類型
abstract class E_InvoiceTagType
{
	// 發票開立
	const Invoice = 'I';
	// 發票作廢
...
{	
	...
	public $HashIV 		= '';
	public $Send 		= array();
	public $Invoice_Method 	= '';	// 電子發票執行項目
	public $Invoice_Url 	= '';	// 電子發票執行網址
	...
	function __construct()
	{
	        ...
	        $this->Send = array(
            "RelateNumber" => '', //會員自訂編號，必填
	            "CustomerID" => '', //客戶代號，CarruerType為1需填
	            "CustomerIdentifier" => '', //統一編號
	            "CustomerName" => '', //客戶名稱，Print為1需填，urlencode
	            "CustomerAddr" => '', //客戶地址，Print為1需填，urlencode
	            "CustomerPhone" => '', //客戶手機號碼，與電子信箱取其一必填
	            "CustomerEmail" => '', //客戶電子信箱，與手機號碼取其一必填，urlencode
	            "ClearanceMark" => '', //通關方式，當TaxType為2時，帶1(海關出口)或2(非海關出口)
        	"Print" => E_PrintMark::No, //不列印或捐贈，Donation為1時，參數帶0；CustomerIdentifier有值時，帶1，必填
            "Donation" => E_Donation::No, //捐贈帶1，不捐贈或CustomerIdentifier有值時，帶0，必填
	            "LoveCode" => '', //愛心碼，Donation唯一需填
		    	"CarruerType" => E_CarruerType::None, //載具類別，會員載具1，自然人憑證2，手機條碼3，CustomerIdentifier有值時，不可為1 or 2
	            "CarruerNum" => '', //載具編號，CarruerType為空或1時，帶空
            "TaxType" => E_TaxType::Dutiable, //課稅類別，應稅1，零稅率2，免稅3，混和9，必填
            "SalesAmount" => '', //發票金額，必填
	            "InvoiceRemark" => '', //備註，urlencode
	            "Items" => array(), //商品
            "InvType" => E_InvType::General, //字軌類別，一般稅額07，特種稅額08，必填
	            "vat" => E_VatType::Yes, //商品單價是否含稅，含稅1，未稅
	            "DelayFlag" => '',
	            "DelayDay" => 0,
	            "ECBankID" => '',
		    	"Tsr" => '',
	...
	*/
	...
	{		
		...
		else
		{
			...
			// A.一般開立發票 
			if( $this->Invoice_Method == E_InvoiceMethod::INVOICE )
			{
				// 3-1過濾不需要的項目
			...
			// B.延遲或觸發開立發票
			if( $this->Invoice_Method == E_InvoiceMethod::INVOICE_DELAY )
			{
				// 3-1過濾不需要的項目
			...
			// C.開立折讓
			if( $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE )
			{
				// 3-1過濾不需要的項目
			...
			// D.發票作廢
			if( $this->Invoice_Method == E_InvoiceMethod::INVOICE_VOID )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['RelateNumber']) ;
				unset($aSend_Info['CustomerID']) ;
			...
			// E.折讓作廢
			if( $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_VOID )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['RelateNumber']) ;
				unset($aSend_Info['CustomerID']) ;
			...
			// F.查詢發票 G.查詢作廢發票
			if( $this->Invoice_Method == E_InvoiceMethod::INVOICE_SEARCH || $this->Invoice_Method == E_InvoiceMethod::INVOICE_VOID_SEARCH )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['CustomerID']) ;
				unset($aSend_Info['CustomerIdentifier']) ;
			...
			// H.查詢折讓明細 I.查詢折讓作廢明細
			if( $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_SEARCH || $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_VOID_SEARCH )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['RelateNumber']) ;
				unset($aSend_Info['CustomerID']) ;
			...
			// J.發送通知
			if( $this->Invoice_Method == E_InvoiceMethod::INVOICE_NOTIFY )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['RelateNumber']) ;
				unset($aSend_Info['CustomerID']) ;
			...
			// K.查詢折讓明細
			if( $this->Invoice_Method == E_InvoiceMethod::INVOICE_TRIGGER )
			{
				// 3-1過濾不需要的項目
				unset($aSend_Info['RelateNumber']) ;
				unset($aSend_Info['CustomerID']) ;
			...
			{
		  		// A. B. C. D. E.
		  		if( $this->Invoice_Method == E_InvoiceMethod::INVOICE || $this->Invoice_Method == E_InvoiceMethod::INVOICE_DELAY || $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE || $this->Invoice_Method == E_InvoiceMethod::INVOICE_VOID || $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_VOID)
	  			{
					// 5-1 CheckMac檢查
				...
				// F.查詢發票
				if( $this->Invoice_Method == E_InvoiceMethod::INVOICE_SEARCH )
	  			{
	  				// 5-1 CheckMac檢查
					if(count($aReturn_Info) > 0 && isset($aReturn_Info['CheckMacValue']))
					{
	  			...
	  			// G.查詢作廢發票
				if( $this->Invoice_Method == E_InvoiceMethod::INVOICE_VOID_SEARCH )
	  			{
	  				// 5-1 CheckMac檢查
					if(count($aReturn_Info) > 0 && isset($aReturn_Info['CheckMacValue']))
					{
	  			...
	  			// H.查詢折讓明細
				if( $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_SEARCH )
	  			{
	  			...
	  			// I.查詢折讓作廢明細
				if( $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_VOID_SEARCH )
	  			{
	  				// 5-1 CheckMac檢查
					if(count($aReturn_Info) > 0 && isset($aReturn_Info['CheckMacValue']))
					{
	...
	*/
	        ...
	        if(strlen($this->Invoice_Url) == 0)
		{
	        ...
	        if( $this->Invoice_Method == E_InvoiceMethod::INVOICE || $this->Invoice_Method == E_InvoiceMethod::INVOICE_DELAY || $this->Invoice_Method == E_InvoiceMethod::INVOICE_SEARCH || $this->Invoice_Method == E_InvoiceMethod::INVOICE_VOID_SEARCH)
	        {	
	        	// 4.廠商自訂編號
	        ...
	        if( $this->Invoice_Method == E_InvoiceMethod::INVOICE || $this->Invoice_Method == E_InvoiceMethod::INVOICE_DELAY )
	        {	
	        	// 5.客戶代號 CustomerID
		...
		// 檢查 A.一般開立發票、B.延遲觸發開立發票
	        ...
	        {		        	
	        	...
	        	if ($this->Send['Print'] == E_PrintMark::Yes)
        		{
			...
			if ($this->Send['TaxType'] == E_TaxType::Zero)
			{
				if ( ( $this->Send['ClearanceMark'] != E_ClearanceMark::Yes ) && ( $this->Send['ClearanceMark'] != E_ClearanceMark::No ) )
				{
			...
			else
			{
	        	...
	        	// *列印註記僅能為 0 或 1
			if ( ( $this->Send['Print'] != E_PrintMark::Yes ) && ( $this->Send['Print'] != E_PrintMark::No ) )
			{
				array_push($arErrors, "12:Invalid Print.");
			}
			// *若捐贈註記 = '1' (捐贈)時，則VAL = '0' (不列印)
			if ($this->Send['Donation'] == E_Donation::Yes)
			{
				if ($this->Send['Print'] != E_PrintMark::No)
				{
			...
			if ( ($this->Send['Donation'] != E_Donation::Yes ) && ( $this->Send['Donation'] != E_Donation::No ) )
			{
				array_push($arErrors, "13:Invalid Donation.");
			}
			// *若統一編號有值時，則VAL = '2' (不捐贈)
			if (strlen($this->Send['CustomerIdentifier']) > 0 && $this->Send['Donation'] == E_Donation::Yes )
			{
			...
			if ( ( $this->Send['CarruerType'] != E_CarruerType::None ) && ( $this->Send['CarruerType'] != E_CarruerType::Member ) && ( $this->Send['CarruerType'] != E_CarruerType::Citizen ) && ( $this->Send['CarruerType'] != E_CarruerType::Cellphone ) )
			{
			...
			else
			{
				...
				if (strlen($this->Send['CustomerIdentifier']) > 0)
				{
					if ($this->Send['CarruerType'] == E_CarruerType::Member || $this->Send['CarruerType'] == E_CarruerType::Citizen )
					{
                          			array_push($arErrors, "15:Invalid CarruerType.");
                      			}		
				}
			...
			switch ($this->Send['CarruerType'])
			{
              			...
              			case E_CarruerType::None:
              			case E_CarruerType::Member:
			...
			if ( ( $this->Send['TaxType'] != E_TaxType::Dutiable ) && ( $this->Send['TaxType'] != E_TaxType::Zero ) && ( $this->Send['TaxType'] != E_TaxType::Free ) && ( $this->Send['TaxType'] != E_TaxType::Mix ) )
			{
		...
		// 檢查 A.一般開立發票、B.延遲觸發開立發票、C.開立折讓
	        if( $this->Invoice_Method == E_InvoiceMethod::INVOICE || $this->Invoice_Method == E_InvoiceMethod::INVOICE_DELAY || $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE )
	        {	
		...
		// 檢查 A.一般開立發票、B.延遲觸發開立發票
			...
			if( ( $this->Send['InvType'] != E_InvType::General ) && ( $this->Send['InvType'] != E_InvType::Special ) )
			{
		...
		if( $this->Invoice_Method == E_InvoiceMethod::INVOICE )
			...
			{
				if( ( $this->Send['vat'] != E_VatType::Yes ) && ( $this->Send['vat'] != E_VatType::No ) )
				{
		...
		// 檢查 B.延遲觸發開立發票
	        ...
	        {
	        	// 30.延遲註記 DelayFlag
			if( ( $this->Send['DelayFlag'] != E_DelayFlagType::Delay ) && ( $this->Send['DelayFlag'] != E_DelayFlagType::Trigger ) )
			{
			...
			if ( $this->Send['DelayFlag'] == E_DelayFlagType::Delay )
			{
			...
			if ($this->Send['DelayFlag'] == E_DelayFlagType::Trigger)
			{
		...
		// 檢查 B.延遲觸發開立發票、K.付款完成觸發或延遲開立發票
	        if( $this->Invoice_Method == E_InvoiceMethod::INVOICE_DELAY || $this->Invoice_Method == E_InvoiceMethod::INVOICE_TRIGGER)
	        {	
			// 33.交易單號 Tsr
			...
			if( ( $this->Send['PayType'] != E_PayTypeCategory::Ecbank ) && ( $this->Send['PayType'] != E_PayTypeCategory::Ecpay ) && ( $this->Send['PayType'] != E_PayTypeCategory::Allpay ) )
			{
		...
		// 檢查 B.延遲觸發開立發票
			...
			switch ($this->Send['PayType'])
			{
              			...
              			case E_PayTypeCategory::Ecbank:
					$this->Send['PayAct'] = 'ECBANK' ;
				break;
                 		 
				// *若交易類別='2'時，則VAL = 'ECPAY'
              			case E_PayTypeCategory::Ecpay:
					$this->Send['PayAct'] = 'ECPAY' ;
				break;
              
              			// *若交易類別='3'時，則VAL = 'ALLPAY'
              			case E_PayTypeCategory::Allpay:
	        ...
	        if( $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE || $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_VOID || $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_SEARCH || $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_VOID_SEARCH || $this->Invoice_Method == E_InvoiceMethod::INVOICE_NOTIFY )
	        {
	        	// 37.發票號碼 InvoiceNo
	        ...
	        {	
	        	...
	        	// *固定給定下述預設值
	        	if( ( $this->Send['AllowanceNotify'] != E_AllowanceE_NotifyType::Sms ) && ( $this->Send['AllowanceNotify'] != E_AllowanceE_NotifyType::Email ) && ( $this->Send['AllowanceNotify'] != E_AllowanceE_NotifyType::All ) && ( $this->Send['AllowanceNotify'] != E_AllowanceE_NotifyType::None ) )
			{
			...
			if(strlen($this->Send['NotifyMail']) > 0 )
	        	...
	        	if($this->Send['AllowanceNotify'] == E_AllowanceE_NotifyType::Email && strlen($this->Send['NotifyMail']) == 0 )
	        	{
	        	...
	        	if (strlen($this->Send['NotifyPhone']) > 20)
			{
	        	...
	        	if( $this->Send['AllowanceNotify'] == E_AllowanceE_NotifyType::Sms && strlen($this->Send['NotifyPhone']) == 0 )
	        	{
	        		array_push($arErrors, "40:NotifyPhone is required.");
	        	}
	        	
	        	// 39-40 通知電子信箱、通知手機號碼不能全為空值 (如果狀態為SMS 或 EMAIL)
	        	if(strlen($this->Send['NotifyPhone']) == 0 && strlen($this->Send['NotifyMail']) == 0 && ( $this->Send['AllowanceNotify'] == E_AllowanceE_NotifyType::Sms || $this->Send['AllowanceNotify'] == E_AllowanceE_NotifyType::Email ) )
	        	{
	        	...
	        	else
	        	{
		        	...
		        	if( $this->Send['AllowanceNotify'] == E_AllowanceE_NotifyType::All && ( strlen($this->Send['NotifyMail']) == 0 || strlen($this->Send['NotifyPhone']) == 0 ) )
		        	{
		        		array_push($arErrors, "39-40:NotifyMail And NotifyPhone is required.");
		        	}
		        	
		        	// *下述情況通知手機號碼與電子信箱為空值(通知類別為N-皆不通知)
		        	if($this->Send['AllowanceNotify'] == E_AllowanceE_NotifyType::None && ( strlen($this->Send['NotifyMail']) > 0 || strlen($this->Send['NotifyPhone']) > 0 ))
		        	{
		...
		// 檢查 D.發票作廢
	        ...
	        // 檢查 D.發票作廢、折讓作廢
	        if( $this->Invoice_Method == E_InvoiceMethod::INVOICE_VOID || $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_VOID )
	        {
			// 43.作廢原因 Reason
		...
		// 檢查 E.折讓作廢、H.查詢折讓明細 I.查詢折讓作廢明細、J.發送通知
	        if( $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_VOID || $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_SEARCH || $this->Invoice_Method == E_InvoiceMethod::ALLOWANCE_VOID_SEARCH || $this->Invoice_Method == E_InvoiceMethod::INVOICE_NOTIFY )
	        {
			// 44.折讓編號 AllowanceNo
		...
		// 檢查J.發送通知
			...
			if(strlen($this->Send['NotifyMail']) > 0 )
	        	...
	        	if( $this->Send['Notify'] == E_NotifyType::Email && strlen($this->Send['NotifyMail']) == 0 )
	        	{
	        		array_push($arErrors, "39:NotifyMail is required.");
	        	}
			// *UrlEncode
			...
			if(strlen($this->Send['Phone']) > 0 )
	        	...
	        	if( $this->Send['Notify'] == E_NotifyType::Sms && strlen($this->Send['Phone']) == 0 )
	        	{
	        	...
	        	{
	        		if( $this->Send['Notify'] == E_NotifyType::All && ( strlen($this->Send['NotifyMail']) == 0 || strlen($this->Send['Phone']) == 0 ) )
	        		{
	        	...
	        	// *固定給定下述預設值
	        	if( ($this->Send['Notify'] != E_NotifyType::Sms ) && ( $this->Send['Notify'] != E_NotifyType::Email ) && ( $this->Send['Notify'] != E_NotifyType::All ) )
	        	{
			...
			// *固定給定下述預設值
			if( ( $this->Send['InvoiceTag'] != E_InvoiceTagType::Invoice ) && ( $this->Send['InvoiceTag'] != E_InvoiceTagType::Invoice_Void ) && ( $this->Send['InvoiceTag'] != E_InvoiceTagType::Allowance ) && ( $this->Send['InvoiceTag'] != E_InvoiceTagType::Allowance_Void ) && ( $this->Send['InvoiceTag'] != E_InvoiceTagType::Invoice_Winning ) )
	        	{
			...
			// *固定給定下述預設值
			if( ( $this->Send['Notified'] != E_NotifiedType::Customer ) && ( $this->Send['Notified'] != E_NotifiedType::vendor ) && ( $this->Send['Notified'] != E_NotifiedType::All ) )
	        	{
